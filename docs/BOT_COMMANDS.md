...
This guide documents the complete user interface keyboard system, multi-language engine, private message commands, automatic join/daily group scanners, scammer keyword trigger detection, advanced admin panel controls, developer branding, and HTML parse mode templates for **@telefraudbot**.

---

## 🌐 Multi-Language Localization System (i18n)

`@telefraudbot` supports full multi-language localization across private chats and group environments:

- **🇺🇸 English (`en`)**: Default international standard.
- **🇧🇩 বাংলা (`bn`)**: Full native Bengali interface and transliteration keyword matching.
- **🇮🇳 हिन्दी (`hi`)**: Full native Hindi interface and transliteration keyword matching.

### Language Selection

1. **Private Chat**: Send `/language` or `/setlang`, or tap `🌐 Language` on the persistent Reply Keyboard.
2. **Group Chat**: Group Admins can send `/setlang` or `/language` directly in the group, or switch language from the `/telefraud_settings` panel.
...
When a user sends `/start` in PM, the persistent bottom Reply Keyboard is activated (localized to user's selected language):
...
- **Scam Accusation Trigger**: `[ 🛡️ Submit Fraud Report ]` (deep links into PM with pre-filled target)
- **Admin Review Queue**: `[ ✅ Approve Fraud ]`, `[ ❌ Reject ]`, `[ ❓ Request Proof ]`, `[ 🚫 Ban Reporter ]`
- **Group Settings Panel**: `[ ⚙️ Auto-Ban: ON ]`, `[ 🗑️ Auto-Delete: ON ]`, `[ 🌐 Language: 🇺🇸 English ]`
- **Language Switcher**: `[ 🇺🇸 English ]`, `[ 🇧🇩 বাংলা (Bengali) ]`, `[ 🇮🇳 हिन्दी (Hindi) ]`
...
| `🌐 Language` (`/language`, `/setlang`)| None                      | Opens interactive inline language selector (English, Bengali, Hindi).                    |
